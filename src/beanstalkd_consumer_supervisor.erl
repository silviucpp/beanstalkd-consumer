-module(beanstalkd_consumer_supervisor).
...
init([PoolName, NumberOfWorkers, Params]) ->
    ...
    Fun = fun(X, Acc) ->
        Name = binary_to_atom(<<"beanstalkd_consumer_", PoolNameBin/binary, (integer_to_binary(X))/binary>>, utf8),
...
children_specs(Name, Args) ->
    {Name, {beanstalkd_consumer, start_link, [Args]}, transient, 2000, worker, [Name]}.